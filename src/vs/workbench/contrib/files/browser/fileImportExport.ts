...
export class FileDownload {
		...
		if (preferFileSystemAccessWebApis && WebFileSystemAccess.supported(window)) {
			try {
				...
				if (stat.isDirectory) {
					...
					await this.downloadFolder(stat, targetFolder, operation, cts.token);
				} else {
					await this.downloadFile(parentFolder, stat, operation, cts.token);
				}
	...
	private async downloadFileBuffered(resource: URI, target: FileSystemWritableFileStream, operation: IDownloadOperation, token: CancellationToken): Promise<void> {
	...
	private async downloadFileUnbuffered(resource: URI, target: FileSystemWritableFileStream, operation: IDownloadOperation, token: CancellationToken): Promise<void> {
	...
	private async downloadFile(targetFolder: FileSystemDirectoryHandle, file: IFileStatWithMetadata, operation: IDownloadOperation, token: CancellationToken): Promise<void> {
		...
		if (file.size > ByteSize.MB) {
			return this.downloadFileBuffered(file.resource, targetFileWriter, operation, token);
		}

		// For small files prefer to write unbuffered to reduce overhead
		return this.downloadFileUnbuffered(file.resource, targetFileWriter, operation, token);
	}

	private async downloadFolder(folder: IFileStatWithMetadata, targetFolder: FileSystemDirectoryHandle, operation: IDownloadOperation, token: CancellationToken): Promise<void> {
		if (folder.children) {
			...
			for (const child of folder.children) {
				...
				if (child.isFile) {
					await this.downloadFile(targetFolder, child, operation, token);
				} else {
					...
					await this.downloadFolder(resolvedChildFolder, childFolder, operation, token);
				}
			}
		}
	}
